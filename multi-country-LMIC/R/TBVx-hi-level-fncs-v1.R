...
setup.model.log <- function(paths) {
  ...
  return(function(level = "INFO", msg = NULL) {
...
modify.input.csv <- function(inputcsvfname = NA, new.parameter.values = NULL) {
  input.csv <- read.csv(inputcsvfname, stringsAsFactors = F, header = T, fileEncoding = "UTF-8-BOM")
  if (!is.null(new.parameter.values)) {
    constant <- constant.parameters(parameters.file = inputcsvfname)
...
update.model.parameters <- function(model.params = NULL, selected.params = NULL) {
  ...
  if (!is.null(selected.params)) {
    model.params$xml$doc <- update.constant.parameters(model.params, selected.params)
  }
  return(model.params)
}
