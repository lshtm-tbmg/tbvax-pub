  ...
  with(model, {
    ...
    for (i in 1:opts$nruns) {
      ...
      local( # RC 18112020 included for generating target and parameter output
        {
          tid <- if (Sys.getenv(opts$taskenvvar) != "") {
            Sys.getenv(opts$taskenvvar)
          } else {
            "TNF"
          }
          ...
          if (!dir.exists(file.path(paths$country.output.dir, "extra_params"))) {
            dir.create(file.path(paths$country.output.dir, "extra_params"), recursive = T)
          }
          if (!dir.exists(file.path(paths$country.output.dir, "extra_targets"))) {
            dir.create(file.path(paths$country.output.dir, "extra_targets"), recursive = T)
          }
...
optimize <- function(fn = NULL, model = NA, control = NA, logfn = NULL, flatmid = 0) {
  nls.lm(
    par = model$new.param.values, lower = model$fitted.params$min, upper = model$fitted.params$max,
