...
popadjrate.from.popadj <- function(p, pop = NULL) {
  assert_that(!is.null(pop), msg = "no pop adj data found in popadjrate.from.popadj")
...
read.incidence <- function(parms, fname = NA, countrycode = NA, proportions = F) {
  ...
  list(
    dim = dim, from = from, to = to, VXa = unique(all.inci$VXa),
    RISK = unique(all.inci$RISK),
    SES = unique(all.inci$SES),
    HIV = unique(all.inci$HIV),
    TB = unique(all.inci$TB),
    inci = adjust.age.groups(p = parms, M = inci, compress.1st = compress, proportions = proportions)
  ) # compress incidence in newborns
}
