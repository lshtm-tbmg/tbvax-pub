...
get.popadjrate <- function(p, t) {
  if (p$intervention & t >= p$intervention.start) {
    return(p$popadjrateintv[p$popadjfnintv(t), ])
  } else {
    return(p$popadjratedem[p$popadjfndem(t), ])
  }
}
...
create.approx.fun <- function(row, df = NULL) {
  s <- as.character(row["values"])
  values <- unlist(strsplit(s, ","))
...
init.constants <- function(p, xml) {
  ...
  with(p, {
    ...
    TB <- xml_attr(xml_find_all(xml$doc, "//TB/TB.stages/stage"), "name")
    nTB <- length(TB)
    HIV <- xml_attr(xml_find_all(xml$doc, "//HIV/HIV.stages/stage"), "name")
    nHIV <- length(HIV)
    RISK <- xml_attr(xml_find_all(xml$doc, "//RISK/RISK.stages/stage"), "name")
    nRISK <- length(RISK)
    SES <- xml_attr(xml_find_all(xml$doc, "//SES/SES.stages/stage"), "name")
    nSES <- length(SES)
    VXa <- xml_attr(xml_find_all(xml$doc, "//VXa/VXa.stages/stage"), "name")
    nVXa <- length(VXa)
    ...
    inci.dim.names <- list(
      VXa = dim.names.for.all(environment(), "VXa"),
      NULL,
      NULL,
    ...
    names(AGES) <- paste("A", AGES, sep = "")
    nAGES <- length(AGES)

    ALIVE <- !grepl(".*dead.*", VXaSESRISKHIVTB, ignore.case = F)

    DEATH.vector <- create.death.vector(environment())
  })
}
  ...
  for (name in names(inci.data)) {
    ...
    if (nrow(df) > 0) {
      ...
      for (i in seq_along(1:nrow(df))) {
        inci[[name]][[i]] <- init.incidence(p,
          fname = here(paths$country.dir, df$file[i]),
...
initialize.TB.transmission <- function(p = NULL) {
  ...
  if (length(xmltransitions) > 0) {
    ...
    Tm <- init.parameters.from.xml(p,
      xmlpath = "//TB/TB.transmission", xmlparameter = "TB.parameter",
...
initialize.TB.infectivity <- function(p = NULL) {
  ...
  Infc <- init.parameters.from.xml(p, xmlpath = "//TB/TB.infectivity", xmlparameter = "TB.parameter", xmltransitions = xmltransitions, values = T)
  optimize.params(Infc)
}
  ...
  if (p$DIMLENGTHS[dim] > 1) {
    ...
    if (length(nodeset) > 0) {
      ...
      for (i in 1:length(nodeset)) {
        tr[[i]] <- init.parameters.from.xml(p,
          xmlpath = xmlpath,
...
read.model.parameters <- function(paths = NULL) {
  ...
  with(p, {
    ...
    if (!is.null(paths[["country.code"]])) {
      run.params$countrycode <- paths$country.code
    }
    ...
    popadjratedem <- popadjrate.from.demography(environment(), fname = popfname, countrycode = run.params$countrycode)
    popadjfndem <- create.approx.fun.for.dataframe(popadjratedem)
...
set.baseline.in.model.parameters <- function(params = NULL, output = NULL) {
  ...
  params$popadjrateintv <- popadjrate.from.popadj(params, pop = output$dfrPOPadj)
  params$bgdeathrate <- popadjrate.from.popadj(params, pop = output$dfrBGx)
  params$bgdeathfn <- create.approx.fun.for.dataframe(params$bgdeathrate)
  params$popadjfnintv <- create.approx.fun.for.dataframe(params$popadjrateintv)
...
initialize.model.parameters <- function(params = NULL) {
  ...
  with(params, {
    ...
    PROGRESSION <- list(HIV = HIVp, HIVtr = HIVtr, RISK = RISKp, SES = SESp, VXa = VXap)
