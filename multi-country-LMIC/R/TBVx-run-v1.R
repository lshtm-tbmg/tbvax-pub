tryrun = function(mdl=NULL,initialized.params=NULL, output.flows=T){
  tryCatch(
    expr = {
      mdl$run.model(initialized.params, output.flows)
    }, 
    error = function(e) {
...
run=function(mdl=NULL,new.parameter.values=NULL,write.to.file=F,write.xml=NULL, output.flows=T,
  ...
  output = tryrun(mdl,initialized.params, output.flows)
