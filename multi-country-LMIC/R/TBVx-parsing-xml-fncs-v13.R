...
check.data.frame.for.duplicates <- function(z, xmlpath) {
  harmless.dups <- which(duplicated(z))
  if (length(harmless.dups) > 0) {
    warning(paste("duplicates found and removed from ", xmlpath, " ; please correct input file", sep = ""))
    z <- z[-harmless.dups, ]
  }
  # now the harmless dups have been removed we may have conflicting duplicates
  y <- z[, -which(names(z) == "value")]
  for (parameter in unique(y$name)) {
    x <- filter(y, name == parameter)
    nrows <- nrow(x)
    for (colname in names(x)) {
      if (colname != "name") {
        nNAs <- sum(is.na(x[, colname]))
        if (nNAs > 0 & nNAs < nrows) {
          stop(paste("conflict at ", xmlpath, " in parameter ", parameter, ": specify dependency on ", colname, " either as NA or specific for all ", colname, sep = ""))
        }
        # else - it would be best to test if all possible combinations have been defined properly ....
      }
    }
  }
  z
        ...
        for (ses in 1:nSES) {
          for (tb in 1:nTB) {
            ...
            if (length(col.names) == 0) {
              ...
              # print(paste("col.names=",col.names,"; length(col.names)=",length(col.names),"; q=",q,"; newq=",newq))
...
rk.parameters <- function(p, xmldoc, xmlschema, path = NA, names = NA, types = NA) {
  for (i in seq_along(names)) {
    nm <- names[i]
    typ <- types[i]
    q <- xml_attr(xml_find_all(xmldoc, path), nm)
    if (is.na(q)) {
...
parse.run.spec <- function(xmlfile = NA) {
  ...
  p$num.int <- rk.parameters(p$num.int, xmldoc, xmlschema, path = "//simulation", names = c("dt"), types = c("double"))
  p$num.int <- rk.parameters(p$num.int, xmldoc, xmlschema,
    path = "//simulation/options/numerical.integration",
    names = c("method", "maxsteps", "hmin", "hini", "atol", "rtol"),
    types = c("text", "integer", "double", "double", "double", "double")
  )
  p$num.int <- rk.parameters(p$num.int, xmldoc, xmlschema,
    path = "//simulation/options/exceptional.conditions",
    names = c("min.value.for.state.var"),
    types = c("double")
  )
  ...
  p$simulation <- simulation.years(xmldoc, p$num.int$dt)
...
get.seeded.infections <- function(xmldoc) {
  ...
  df$fraction <- df$fraction * multiplier
  assert_that(sum(df$fraction) <= 1, msg = "multiplier too large, sum of initial TB stage fractions > 1")
  uninfected.stage <- xml_attr(xml_find_all(xmldoc, "//TB/TB.stages/stage")[1], "name")
  fraction.uninfected <- 1 - sum(df$fraction)
  z <- rbind(df, data.frame(stage = uninfected.stage, age.from = df$age.from[1], age.thru = df$age.thru[1], fraction = fraction.uninfected))
  z
}
...
output.options.from.xpath <- function(xmldoc, xpath) {
  ...
  if (n > 0) {
    Tm <- rep(F, n)
    Tm[xml_attr(nodeset, "transmission") == "true"] <- T
    Pr <- rep(F, n)
    Pr[xml_attr(nodeset, "progression") == "true"] <- T
    Ix <- rep(F, n)
    Ix[xml_attr(nodeset, "incidence") == "true"] <- T
    Tr <- rep(F, n)
    Tr[xml_attr(nodeset, "treatment") == "true"] <- T
...
parse.incidence.output.spec <- function(xmldoc) {
  ...
  df <- data.frame(
    dim = xml_attr(include.output, "name"),
