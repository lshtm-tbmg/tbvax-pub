...
cc_all <- read.csv("/n/holyscratch01/menzies_lab/Lab/AllisonEconOutput/data/cc_all_25pct.csv",header=TRUE,check.names=FALSE)
oop_all <- read.csv("/n/holyscratch01/menzies_lab/Lab/AllisonEconOutput/data/oop_all_25pct.csv",header=TRUE,check.names=FALSE)
...
# calc all results then separate step for summary statistics
  ...
  write.csv(output_all,paste0("/n/holyscratch01/menzies_lab/Lab/AllisonEconOutput/equity_25pct/",i,"_equity.csv"),row.names=FALSE)
  rm(output)
