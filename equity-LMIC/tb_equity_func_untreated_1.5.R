...
cc_all_1.5 <- read.csv("/n/holyscratch01/menzies_lab/Lab/AllisonEconOutput/data/cc_all_1.5.csv",header=TRUE,check.names=FALSE)
oop_all_1.5 <- read.csv("/n/holyscratch01/menzies_lab/Lab/AllisonEconOutput/data/oop_all_1.5.csv",header=TRUE,check.names=FALSE)
...
# second loop: uid
  ...
  for (j in UID_list) {
    ...
    cc_cty_1.5 <- cc_all_1.5[which(cc_all_1.5$code==i),]
    oop_cty_1.5 <- oop_all_1.5[which(oop_all_1.5$code==i),]
...
# calc all results then separate step for summary statistics
    ...
    for (k in scenarios) {
      ...
      cc3 <- c(cc_cty_1.5[counter,"cc_poorest"],cc_cty_1.5[counter,"cc_poorer"],cc_cty_1.5[counter,"cc_middle"],cc_cty_1.5[counter,"cc_richer"],cc_cty_1.5[counter,"cc_richest"])
      cc4 <- rep(NA,5)
      v7 <- rep(cc3,each=1e5/5)
      for(l in 1:5) cc4[l] <- mean(v7[id[l]:id[l+1]])
      
      # % untreated experiencing catastrophic direct medical costs
      oop3 <- c(oop_cty_1.5[counter,"oop_poorest"],oop_cty_1.5[counter,"oop_poorer"],oop_cty_1.5[counter,"oop_middle"],oop_cty_1.5[counter,"oop_richer"],oop_cty_1.5[counter,"oop_richest"])
      oop4 <- rep(NA,5)
  ...
  write.csv(output_all,paste0("/n/holyscratch01/menzies_lab/Lab/AllisonEconOutput/equity_1.5/",i,"_equity.csv"),row.names=FALSE)
  rm(output)
