  ...
  if (t>p$run.params$simulation$years[1]){
    ...
    for (i in seq_along(p$inci$VXa)){
      if (p$inci$VXa[[i]]$inci.once.per.year & t>=p$intervention.start & p$inci$VXa[[i]]$inci.fn(t+offset)>0){
        ...
        colnames(dvacc)=as.integer(substring(colnames(dvacc),2)) 
...
run.deSolve = function(params = NULL){
  ...
  rawout = rk(   y = as.vector(params$y.ini), 
             ...
             hmax     = params$run.params$num.int$hmax,
             # mf = 10,
...
demography.from.model.run=function(out,params){
  ...
  if (!params$intervention){  
    for (i in seq_along(t)){ 
      ...
      dBGx[i,]      = get.deathrate.allcauses(params,t[i]) * Y[i,] - dHIVTBx[i,]
      n = sum(dBGx[i,]<0)
      if (n>0){
        modlog(level='DEBUG',msg=paste0("at t=",t[i]," ",n," age groups with HIV TB deaths exceeding all cause mortality (UNPOP data) ; capping to ensure background deaths >= 0"))
      }
      ...
      if (i>1){
        dPOPadj[i,] = as.numeric(params$popadj[as.integer(rownames(params$popadj))==trunc(t[i]),])
      }else{
    ...
    for (i in seq_along(t)){
      Y[i,]         = colSums(out$state[[indexes[i]]][params$ALIVE,])
      dHIVTBx[i,]   = out$dHIVTBx[[indexes[i]]]
      dBGx[i,]      = get.bgdeathrate(params,t[i]) * Y[i,]
  ...
  if (!params$intervention){
    dPOPadj.df = cbind(year=trunc(t),country=country,as.data.frame(dPOPadj,row.names = F))
  }else{
    dPOPadj.df = NULL
  }
