...
## -p Parameter file including .csv extension (Default input_FM.csv)
## -t Target file including .csv extentson (Default target_em.csv)
...
tfile <- if(is.null(opts$targets)) "target_em.csv" else opts$targets
pfile <- if(is.null(opts$parameters)) "input_FM.csv" else opts$parameters
