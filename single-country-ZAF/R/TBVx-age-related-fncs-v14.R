...
matmul.by.age.group.in.out=function(subject,Y,mult=NA){
...
matmul.by.age.group=function(subject,Y,mult=1){
  X = subject$parameter.matrices
  if (assert){
    if (is.null(subject$age.ranges))
      stop("age ranges should not be NULL")
    agelim = c(subject$age.groups,200)
    nag = length(agelim)-1
    # print("assertions are active in matmul.by.age.group")
    if (nag>1 & (prod(agelim>=0)!=1 | prod((agelim[2:(nag+1)]-agelim[1:nag])>0) !=1))
      stop("all age limits should be >= 0 and strictly increasing")
    #if (class(Y)!="matrix")
    #  stop("expecting a matrix for state variables")
    if (nrow(X[[1]]) != nrow(Y))
      stop(paste("X should be a matrix with the same nr of rows as Y"))
  }
  nag   = length(subject$age.groups)
  if (nag==1){
    return(mult*as.matrix(X[[1]]%*%Y))
  }else{
    dY = matrix(0.,nrow(Y),ncol(Y))
    for (i in 1:nag){
      range = subject$age.ranges[[i]]
      dY[,range] = as.matrix(X[[i]] %*% Y[,range]) 
    }
    return(mult*dY)
  }
}
