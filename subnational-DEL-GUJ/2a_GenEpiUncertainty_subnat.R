...
if (!dir.exists(here("epi_output/grouped_output", cc))) dir.create(here("epi_output/grouped_output", cc))

# Load in data generated from GenEpiOutput_complex.R
n_epi <- open_dataset(sources = paste0("./epi_output/n_epi/", cc)) %>% dplyr::collect()
...
# (subset to closer before vx introduced)
...
fwrite(raw_plots_long, paste0("epi_output/grouped_output/", cc, "/raw_output.csv"))
...
fwrite(sum_averted, paste0("./epi_output/grouped_output/", cc, "_casesdeaths_averted.csv"))
...
fwrite(relative_output, paste0("epi_output/grouped_output/", cc, "/relative_output.csv"))
