...
tbmacro <- function(index){
  ...
  alpharaw <- read.csv("/n/holyscratch01/menzies_lab/Lab/AllisonEconOutput/data/LAP_2GDP_NOC_RT_A.csv",header=TRUE,check.names=FALSE)
  alphadata <- alpharaw[alpharaw$ref_area %in% lcountries,]
  ...
  lfpr0raw <- read.csv("/n/holyscratch01/menzies_lab/Lab/AllisonEconOutput/data/EAP_DWAP_SEX_AGE_RT_A.csv",header=TRUE,check.names=FALSE)
  classif1_list <- c("AGE_5YRBANDS_Y15-19", "AGE_5YRBANDS_Y20-24",
                                      ...
                                      "AGE_5YRBANDS_YGE65")
  lfpr0full <- lfpr0raw[lfpr0raw$ref_area %in% lcountries,]
  lfpr0full <- lfpr0full[lfpr0full$classif1 %in% classif1_list,]
  lfpr0full <- lfpr0full[lfpr0full$time<=2020,]
  ...
  directory <- paste0("/n/holyscratch01/menzies_lab/Lab/MacroRevision/tbvax/econ_output_endTB/")
  ...
  country_cost0 <- read.csv(paste0("/n/holyscratch01/menzies_lab/Lab/AllisonEconOutput/outcomes/",i,"_outcomes_macro_endtb.csv"),header=TRUE,check.names=FALSE)
  
  ## Read in inc YLD data (for all UIDs) # NAM edits Oct 31 2022
  load(paste0("/n/holyscratch01/menzies_lab/Lab/AllisonEconOutput/yld/",i,"_yld_endTB.rData")) # country_inc_yld
  ...
  for (j0 in 1:length(UID_list)) { 
    ...
    # Define list of vaccine delivery scenarios # ACP edits March 3 2023
    scenarios <- unique(country_TB$Runtype)
    scenarios <- subset(scenarios,scenarios!="baseline")
    ...
    for (k in 1:length(scenarios)) {
      ...
      lfpr1 <- lfpr0full[which(lfpr0full$time==latestyear),]
      lfpr1 <- lfpr1 %>% select(ref_area, classif1, obs_value)
      # lfpr1 <- get_ilostat(
      #   id = c("EAP_DWAP_SEX_AGE_RT_A"),
      #   filters = list(
      #     ref_area = i_temp,
  ...
  write.csv(output_all,paste0("/n/holyscratch01/menzies_lab/Lab/AllisonEconOutput/macro/",i,"_macro_endtb.csv"),row.names=FALSE)
